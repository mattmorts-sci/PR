...
invalid amino acids, output was written to {DATE}_\
{data_source}_cleaned.fasta. {d_count} sequences were dropped \
and written to output/{DATE}_{data_source}_cleaner_\
...
def reviewed_unreviewed(file, file2, data_source):
    """
    ...
    # Create a file with the cleaned sequences
    with open(
        f"output/{DATE}_{data_source}_rev_unrev_deduped.fasta",
        "w+",
    ) as output_file:
        # Just read the hash table and write on the file as a fasta format
        # as key and variable
    ...
    # taking the sequences from the 'duplicate' dict.
    with open(
        f"output/{DATE}_{data_source}_rev_unrev_dropped.fasta", "w+"
    ) as dropped_file:
        for sequence in duplicate:
            dropped_file.write(">" + duplicate[sequence] + "\n" + sequence + "\n")
            d_count += 1
...
output/{DATE}_{data_source}_rev_unrev_deduped.fasta. {d_count} \
sequences were dropped and written to output/{DATE}_{data_source}\
_rev_unrev_dropped.fasta"
    )
    print(
        f"{len(sequences)} sequences were kept, output was written to \
{DATE}_{data_source}_rev_unrev_deduped.fasta. {d_count} sequences were \
dropped and written to output/joint_cleaned_cleaner_dropped_seq.fasta"
    )

    seq_file = f"output/{DATE}_{data_source}_rev_unrev_deduped.fasta"
    print("")
    print(seq_file)
