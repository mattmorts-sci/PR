...
def annotations(file, data_source, delim="|", el_num=1):
    """
...
database_pfam\n"
    ...
    with open(
        f"output/{date}_{data_source}_filt_\
annotations.txt",
        "w+",
    ) as anno_test:
...
from {file} and outputted to 'output/{date}_{data_source}_filt_\
annotations.txt'"
    )
    log(
        f"Annotations were retrieved or attempted for {anno_count} sequences \
...
def indexing(anno_file, seq_file, delim="|", el_num=1):
    """
    ...
    output_file = f"output/{date}_PFAM_master_index.csv"
...
def reviewed(file):
    ...
    output_file = f"output/{date}_reviewed.fasta"
    # Runs the dict_to_fasta function in the utilities module
    dict_to_fasta(reviewed_dict, output_file)

    # Unreviewed sequences
    # Variable with the name of the outputted fasta file
    output_file = f"output/{date}_unreviewed.fasta"
    # Runs the dict_to_fasta function in the utilities module
    dict_to_fasta(unreviewed_dict, output_file)

    log(f"{rev} sequences were written to output/{date}_reviewed.fasta")
    log(f"{unrev} sequences were written to output/{date}_unreviewed.fasta")

    print(f"{rev} sequences were written to output/{date}_reviewed.fasta")
    print(f"{unrev} sequences were written to output/{date}_unreviewed.fasta")
